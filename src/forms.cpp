...
ValuePtr defineForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    if (args.size() <= 2) {
        throw LispError("Nothing to define");
    }
    if (args.size() == 3) {
    ...
    if (auto name = args[1]->asSymbol()) {  // 定义变量
        ...
        auto functionName =
            std::dynamic_pointer_cast<PairValue>(args[1])->getLeft();
...
bool change_to_bool(ValuePtr ptr, EvalEnv& env) {
    if (env.eval(ptr)->getType() == ValueType::BOOLEAN_VALUE &&
        !std::dynamic_pointer_cast<BooleanValue>(env.eval(ptr))->getValue()) {
        return false;
    } else {
        return true;
    }
...
/**
 * @brief
 * 实现and表达式，逐个检查条件，如果所有条件都为真，则返回true；否则返回false。
 *
...
/**
 * @brief
 * 实现or表达式，逐个检查条件，如果有一个条件为真，则返回该条件的值；否则返回false。
 *
 * @param args 参数列表，包含多个条件
 * @param env 求值环境
...
ValuePtr orForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    for (int i = 1; i < static_cast<int>(args.size()) - 1; i++) {
        if (change_to_bool(args[i], env)) return env.eval(args[i]);
    }
    return std::make_shared<BooleanValue>(false);
}
 ...
 * @return 根据条件执行的结果
 * @throws LispError
 * 如果参数数量不符合规范或条件不是符号或else不是在最后一个分支
 */
...
ValuePtr letForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    ...
    for (int i = 0; i < static_cast<int>(params_args.size()) - 1; i++) {
        ...
        if (vec.size() <= 2) {
            ...
            throw LispError(
                "The param list need param name and value, given 3 "
                "object.");
        }
    }
...
ValuePtr makeList(std::vector<ValuePtr>& vec, int start) {
    ...
    if (start == vec.size() - 2) {
        ...
        return std::make_shared<PairValue>(vec[start],
                                           makeList(vec, start + 1));
    }
}
}  // namespace
...
ValuePtr quasiquoteForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    ...
    if (vec.size() == 0) {
        ...
        for (int i = 0; i < static_cast<int>(vec.size()) - 1; i++) {
            ...
            if (vecVec.size() == 3 && vecVec[0]->isSymbol() &&
                *vecVec[0]->asSymbol() == "unquote") {
